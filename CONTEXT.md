...
**What changed:**
- Ran the user's actual `RESUME.pdf` (real CV, real GitHub username `jasondhaki`, a realistic Upwork paste) through the full pipeline for the first time — the long-pending item at the top of this file, unblocked now that the local-Ollama backend removes the Gemini rate-limit obstacle. Confirmed working two ways: a direct-Python pipeline run (bypassing the browser, since Playwright's own navigation-timeout handling turned out to be unreliable for multi-minute real requests) and, more importantly, the user's own live browser session — readiness 53/100 from 31 real claims across all 3 sources, screenshots reviewed directly.
- Found a real bug in the process: `ollama_client.py`'s 300s timeout was sized against the 3-block smoke-test fixture. The real resume produced 59 text blocks; since the extraction schema makes the model emit one JSON array item per block, output length (and CPU-bound generation time) scales with document size, not prompt length. The CV step alone took 182–356s across repeated real runs (real run-to-run variance exists even at `temperature: 0` — CPU multi-threaded inference isn't perfectly deterministic). Bumped the timeout to 900s.
- Refactored `/analyze` in `main.py` to fetch GitHub concurrently (via a small `ThreadPoolExecutor`) instead of strictly after CV+Upwork — GitHub is a plain network call with no dependency on the other two, so there was no reason to make it wait its turn. Did NOT parallelize CV and Upwork against each other: both hit the same local model instance, which serializes requests anyway, so it wouldn't have overlapped any real work.
- Cleaned up an orphaned `llama-server` process from an earlier aborted test (Playwright's browser died mid-navigation, but the FastAPI sync handler doesn't support cancellation on client disconnect, so the server-side computation kept running for nothing) — a good reminder that killing the *client* of a long sync request doesn't stop the *server's* work.
- User feedback after reviewing their own real result: the T1–T8 tier badges appear everywhere but were never explained anywhere readable (only hover tooltips on the index page), and the ranked "Where to focus next" gap list said *what* to improve but not *how*. Added `app/templates/_tier_legend.html` (shared partial, all 8 tiers with weight + a one-line description of what actually qualifies), included on both the result page and `/benchmark`. Added `GAP_GUIDANCE` to `app/scoring/dimensions.py` — one grounded sentence per dimension describing what its `compute_*` function actually checks, kept in the same file as the formulas so they can't silently drift apart; exposed to templates via a Jinja global rather than threading through both routes that render `result.html`.

**Why:**
- The timeout bug is the kind of thing that would only ever surface on a real document, never on the tiny synthetic fixtures used for iteration — direct confirmation that the CLAUDE.md discipline ("at least one real end-to-end call before calling a phase done") earns its keep.
- Guidance was requested as static, grounded-in-the-real-formula text rather than dynamically-computed specifics (e.g., "you're missing exactly these 3 skills") — a reasonable, lower-risk scope for now; the dynamic version is a real future enhancement, not implemented.

**Next steps:**
- None blocking. If `completeness`'s gap guidance ever feels too generic, it's the one dimension with a fully deterministic checklist (`compute_completeness` in `dimensions.py`) — a natural candidate for showing the exact missing items instead of a generic sentence, if that's ever wanted.
